#  Script name:    ChangeTSProfileLocations.ps1
#  Created on:     04-12-2013
#  Author:         Richard Horsley
#  Purpose:        Bulk replaces TS profile location paths in AD user accounts.
 
# Set the location of the log file for this script.
$outputfile = "C:\Scripts\ChangeTSProfileLocations.txt"
...
$oldtsprofilepath = "\\oldserver\TS Profiles\"
$newtsprofilepath = "\\newserver\TS Profiles\"
...
foreach ($result in $results) {
    ...
    try {
        $tsProfilePath = $user.PSBase.InvokeGet("TerminalServicesProfilePath")
    ...
    $oldtsprofilepath = $oldtsprofilepath + "*"
 
    if ($tsProfilePath -like $oldtsprofilepath) {
 
        echo "User $($user.sAMAccountName) has a TS profile located at: $tsProfilePath" >> $outputfile
        
        $userDN = "$($user.DistinguishedName)"
        $ppValue = "$newtsprofilepath$($user.sAMAccountName)"
